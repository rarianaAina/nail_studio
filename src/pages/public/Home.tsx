...
import {
  CalendarHeart, Sparkles, Heart, Award, Leaf, Clock,
  Phone, MapPin, Star, ArrowRight, Check,
} from 'lucide-react';
    ...
    <div className="overflow-hidden">
      ...
      <section className="relative min-h-[92vh] gradient-rose">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-4 pb-20 pt-32 sm:px-6 lg:grid-cols-2 lg:px-8 lg:pt-40">
          ...
          >
            <Badge variant="secondary" className="mb-6 gap-1.5 rounded-full border border-primary/20 bg-white/70 px-4 py-1.5 text-xs font-medium text-primary backdrop-blur">
            ...
            <div className="mt-10 flex items-center justify-center gap-4 sm:gap-6 lg:justify-start">
              <div className="flex -space-x-3">
                {[3997389, 3997391, 704815].map((id) => (
                  <img key={id}
                    ...
                    alt="Cliente" className="h-10 w-10 rounded-full border-2 border-white object-cover"
                  />
                ))}
              </div>
              <div className="text-left">
                ...
                <p className="text-xs text-muted-foreground">+450 clientes satisfaites</p>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }} className="absolute left-2 top-12 rounded-2xl bg-white/90 p-3 shadow-soft backdrop-blur sm:-left-8">
              <div className="flex items-center gap-2">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-primary/10 text-primary"><Award className="h-4 w-4" /></span>
                <div><p className="text-xs font-semibold">Produits premium</p><p className="text-[10px] text-muted-foreground">Vegan & cruelty-free</p></div>
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 }} className="absolute right-2 bottom-16 rounded-2xl bg-white/90 p-3 shadow-soft backdrop-blur sm:-right-8">
              <div className="flex items-center gap-2">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-accent/15 text-accent"><Clock className="h-4 w-4" /></span>
                <div><p className="text-xs font-semibold">Réservation 24/7</p><p className="text-[10px] text-muted-foreground">En ligne, simple</p></div>
              </div>
            </motion.div>
          </motion.div>
        </div>
      ...
      <section className="py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div {...fadeUp} className="mx-auto max-w-3xl text-center">
            <p className="text-sm font-medium uppercase tracking-[0.25em] text-primary">Notre maison</p>
          ...
          <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <motion.div key={s.l} {...fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }} className="rounded-2xl border border-border/60 bg-card p-6 text-center shadow-soft">
      ...
      <section className="bg-secondary/40 py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div {...fadeUp} className="mx-auto max-w-3xl text-center">
            <p className="text-sm font-medium uppercase tracking-[0.25em] text-primary">Pourquoi nous choisir</p>
            <h2 className="mt-3 font-display text-4xl font-semibold text-foreground sm:text-5xl">Une expérience beauté unique</h2>
          </motion.div>
          <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {[
              { icon: Award, title: 'Expertise certifiée', text: "Des prothésistes diplômées et formées aux dernières techniques internationales." },
              { icon: Leaf, title: 'Produits premium', text: "Vernis et gels vegan, sans danger pour vos ongles ni pour l'environnement." },
              { icon: Sparkles, title: 'Hygiène irréprochable', text: "Stérilisation aux normes hospitalières et matériel à usage unique." },
              { icon: Heart, title: 'Accueil chaleureux', text: "Une ambiance cosy, thé offert, musique douce. Vous êtes notre priorité." },
              { icon: CalendarHeart, title: 'Réservation flexible', text: "Réservez en ligne 24/7, modifiez ou annulez en quelques clics." },
              { icon: Check, title: 'Satisfaction garantie', text: "Retouches offertes sous 48h si le résultat ne vous convient pas." },
            ].map((f, i) => (
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div {...fadeUp} className="flex flex-col items-center justify-between gap-6 sm:flex-row sm:items-end">
            <div className="max-w-2xl">
              <p className="text-sm font-medium uppercase tracking-[0.25em] text-primary">Nos prestations</p>
              <h2 className="mt-3 font-display text-4xl font-semibold text-foreground sm:text-5xl">Des soins pour chaque envie</h2>
            </div>
            <Button asChild variant="outline" className="rounded-full">
              <Link to="/prestations">Voir tout <ArrowRight className="ml-2 h-4 w-4" /></Link>
            </Button>
          </motion.div>
      ...
      {galleryItems.length > 0 && (
        <section className="bg-secondary/40 py-20 sm:py-28">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <motion.div {...fadeUp} className="flex flex-col items-center justify-between gap-6 sm:flex-row sm:items-end">
              <div>
                <p className="text-sm font-medium uppercase tracking-[0.25em] text-primary">Galerie</p>
                <h2 className="mt-3 font-display text-4xl font-semibold text-foreground sm:text-5xl">Nos réalisations</h2>
              </div>
              <Button asChild variant="outline" className="rounded-full">
                <Link to="/galerie">Voir tout <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
            </motion.div>
            <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {galleryItems.slice(0, 10).map((g, i) => (
                <motion.div key={g.id} {...fadeUp} transition={{ duration: 0.4, delay: (i % 5) * 0.07 }}
                  className="group relative overflow-hidden rounded-2xl shadow-soft"
                >
                  <img src={g.image} alt={g.title} className="aspect-square w-full object-cover transition-transform duration-700 group-hover:scale-105" />
                  <div className="absolute inset-0 flex items-end bg-gradient-to-t from-foreground/70 via-foreground/10 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    <p className="p-3 text-sm font-medium text-white">{g.title}</p>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      {reviews.length > 0 && (
        <section className="py-20 sm:py-28">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <motion.div {...fadeUp} className="mx-auto max-w-3xl text-center">
              <p className="text-sm font-medium uppercase tracking-[0.25em] text-primary">Témoignages</p>
              <h2 className="mt-3 font-display text-4xl font-semibold text-foreground sm:text-5xl">Ce qu'elles disent de nous</h2>
            </motion.div>
            <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {reviews.map((r, i) => (
                <motion.div key={r.id} {...fadeUp} transition={{ duration: 0.5, delay: i * 0.08 }}>
      ...
      {/* CTA */}
      <section className="gradient-rose py-20 sm:py-28">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          <motion.div {...fadeUp}>
            <Badge variant="secondary" className="mb-6 gap-1.5 rounded-full border border-primary/20 bg-white/70 px-4 py-1.5 text-xs font-medium text-primary backdrop-blur">
              <CalendarHeart className="h-3.5 w-3.5" /> Réservation en ligne
            </Badge>
            <h2 className="font-display text-4xl font-semibold text-foreground sm:text-5xl">
              Prête pour des ongles sublimes ?
            </h2>
            <p className="mx-auto mt-5 max-w-xl text-foreground/70">
              Réservez votre prochain rendez-vous en quelques clics, 24h/7j.
            </p>
            <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
              <Button asChild size="lg" className="rounded-full px-8 shadow-glow">
                <Link to="/reservation"><CalendarHeart className="mr-2 h-4 w-4" /> Réserver maintenant</Link>
              </Button>
              {settings && (
                <Button asChild size="lg" variant="outline" className="rounded-full bg-white/60 px-8 backdrop-blur">
                  <a href={`tel:${settings.phone}`}><Phone className="mr-2 h-4 w-4" /> {settings.phone}</a>
                </Button>
              )}
            </div>
            {settings && (
